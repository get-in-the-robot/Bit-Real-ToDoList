    ...
    <mdb-card-body>
        ...
        <mdb-list-group>
            ...
            <form action="/task/{{ $task->id }}" method="post">
                @csrf
                @method('PATCH') 
                <mdb-list-group-item class="w-100 mx-auto"  tag="button" type="submit" ><span style='{{ $task->completed == false ? "" : "text-decoration: line-through;" }}'>{{ $task->name }}</span></form>
                    <form action="/task/{{ $task->id }}" method="post">
                        @csrf
                        @method('DELETE')
                        <mdb-btn color="danger" type="submit" style="text-decoration: none">Smazat</mdb-btn></form> </mdb-list-group-item>
